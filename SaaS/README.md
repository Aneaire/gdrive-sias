# g-customize — Packaged Product Plan
...
| `MIGRATION.md` | How the new packaged product relates to the existing single-operator deployment. |
...
- **Existing single-operator deployment**: untouched. The packaged product is a **branch + fresh Convex**, not a migration of the live operator. See `MIGRATION.md`.
...
- Migrating the current single-operator data into the packaged product (deferred indefinitely; `MIGRATION.md`).