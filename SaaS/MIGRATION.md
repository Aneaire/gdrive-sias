# Migration: existing single-operator deployment vs. packaged product

## Decision

The existing single-operator deployment stays on the **current branch**. The packaged product is a **new branch + a fresh Convex deployment**. **No migration of live data.**
...
- We can write a copy-from-old-to-new script later if migration is ever needed.

## Concrete steps

1. **Branch the monorepo** at the current revision.
   - `main` keeps pointing at the existing single-operator code path; legacy deployment continues running. Tag `last-single-operator` to be explicit.
   - New branch `packaged` gets all the SaaS work.
...
4. The legacy branch keeps the CLI and env-var auth flow; the legacy deployment never knows anything changed.
...
## Cut-over of legacy deployment to packaged product (deferred)
...
2. Write `scripts/copy-legacy-to-packaged.mjs` that:
   ...
   - Inserts a `tenants` row for the legacy operator.
